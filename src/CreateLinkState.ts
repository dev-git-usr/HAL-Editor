...
import { HAL_connect } from './haldata/connector';
		...
		this.registerAction(
			new Action({
				...
				fire: (actionEvent: ActionEvent<MouseEvent>) => {
					...
					const ox = this.engine.getModel().getOffsetX() + this.engine.canvas.getBoundingClientRect().left;
					const oy = this.engine.getModel().getOffsetY() + this.engine.canvas.getBoundingClientRect().top;
					...
					if (element instanceof PortModel && !this.sourcePort) {
                        ...
                        */
						...
						if (this.sourcePort.canLinkToPort(element)) {
							...
							this.link.remove();
							HAL_connect(this.sourcePort.getNode().options.name + "." + this.sourcePort.options.name, element.getNode().options.name + "." + element.options.name);
